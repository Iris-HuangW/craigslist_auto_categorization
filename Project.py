...
# split target data as holdout set
...
vec = TfidfVectorizer(analyzer='word', tokenizer=tk, preprocessor=tk, token_pattern=None,
                      min_df=2, ngram_range=(1,6), stop_words='english')
vec.fit(train_x)
...
RFmodel = RandomForestClassifier(n_estimators=200, max_depth=100, bootstrap=True, random_state=0)  # with 10 trees
...
# Deep learning
#DLmodel = MLPClassifier(solver='lbfgs', hidden_layer_sizes=(5, ), random_state=1)
...
df_holdout_pred.to_csv('data/df_holdout_pred.csv',
